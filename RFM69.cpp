...
// set the IRQ pin (which is connected to RFM69's DIO0)
void RFM69::setInterruptPin(uint8_t newInterruptPin)
{
  _interruptPin = newInterruptPin;
}

// get the IRQ pin (which is connected to RFM69's DIO0)
uint8_t RFM69::getInterruptPin()
{
  return _interruptPin;
}

// set the IRQ pin number (on STM32 same as interrupt pin)
void RFM69::setInterruptNumber(uint8_t newInterruptNumber)
{
  _interruptNum = newInterruptNumber;
}

// get the IRQ pin number (on STM32 same as interrupt pin)
uint8_t RFM69::getInterruptNumber()
{
...
{
  return readReg(REG_SYNCVALUE2);
}

// just some Test Pin (STM32F103C8 PC13)
uint8_t RFM69::getTestPin()
{
  return Test_Pin;
}
...
void RFM69::setSlaveSelectPin(uint8_t newSPISlaveSelect)
{
...
// get the slave select (CS) pin 
uint8_t RFM69::getSlaveSelectPin()
{
